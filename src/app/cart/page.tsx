...
const CartPage = () => {
  ...
  const handleIncrement = (id: string) => {
    setCartItems((prevItems) =>
      prevItems.map((item) =>
        item._id === id ? { ...item, stockLevel: item.stockLevel + 1 } : item
      )
    );
    updateCartQuantity(id, (cartItems.find((item) => item._id === id)?.stockLevel || 0) + 1);
  };
  ...
  const handleDecrement = (id: string) => {
    ...
    updateCartQuantity(id, (cartItems.find((item) => item._id === id)?.stockLevel || 1) - 1);
  };
